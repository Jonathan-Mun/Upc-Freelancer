...
function uploadFile(array $file, string $folder, array $allowed = ['jpg','jpeg','png','pdf'], int $maxMb = 5): string|false {
    ...
    if (!is_dir($uploadDir)) {
        if (!mkdir($uploadDir, 0755, true) && !is_dir($uploadDir)) return false;
    }
    if (!is_writable($uploadDir)) return false;
    ...
    // MIME attendus par extension (au lieu d'une liste globale unique)
    $mimeMap = [
        'jpg'  => ['image/jpeg'],
        'jpeg' => ['image/jpeg'],
        'png'  => ['image/png'],
    ...
    if (!isset($mimeMap[$ext]) || !in_array($mime, $mimeMap[$ext])) return false;
