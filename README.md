...
```bash
cp -r upc_freelance/ /upc_freelance/
```

### 2. Créer la base de données
```bash
mysql -u root -p < /upc_freelance/database.sql
```
Ou via phpMyAdmin : importer `database.sql`

### 3. Configurer la connexion BDD
Éditer `/upc_freelance/includes/db.php` :
```php
...
```bash
chmod -R 755 /upc_freelance/storage/
chown -R www-data:www-data /upc_freelance/storage/
```
