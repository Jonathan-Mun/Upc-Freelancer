<?php
// ============================================================
// UPC FREELANCE — Gérer un contrat (litige, annulation, plainte)
// ../../app/contracts/manage.php
// ============================================================

require_once '../../includes/middleware.php';
require_once '../../includes/auth.php';
require_once '../../includes/functions.php';
require_once '../../includes/db.php';

requireLogin();

$user       = currentUser();
$pdo        = getDB();
$contractId = (int)($_GET['id'] ?? 0);
if (!$contractId) redirect('../../app/contracts/list.php');

$stmt = $pdo->prepare('
    SELECT c.*, p.title AS project_title
    FROM contracts c JOIN projects p ON p.id = c.project_id
    WHERE c.id = ? AND (c.client_id = ? OR c.freelancer_id = ?)
');
$stmt->execute([$contractId, $user['id'], $user['id']]);
$contract = $stmt->fetch();
if (!$contract) { http_response_code(403); die('Accès refusé.'); }

$isClient = $user['id'] === $contract['client_id'];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    verifyCsrf();
    $action = sanitize($_POST['action'] ?? '');
    $reason = sanitize($_POST['reason'] ?? '');

    // ─── Ouvrir un litige ──────────────────────────────────
    if ($action === 'dispute' && $contract['status'] === 'active') {
        $pdo->prepare('
            UPDATE contracts SET status = "disputed", disputed_by = ?, dispute_reason = ?
            WHERE id = ?
        ')->execute([$user['id'], $reason, $contractId]);

        $receiverId = $isClient ? $contract['freelancer_id'] : $contract['client_id'];
        sendNotification($receiverId, 'dispute_opened', 'Litige ouvert',
            'Un litige a été ouvert sur le contrat "' . $contract['project_title'] . '". Raison : ' . $reason,
            '/upc_freelance/app/contracts/details.php?id=' . $contractId);
        flash('warning', 'Litige ouvert. Notre équipe va examiner la situation.');
    }

    // ─── Annuler (client seulement) ────────────────────────
    if ($action === 'cancel' && $contract['status'] === 'active' && $isClient) {

        // Vérifier si le freelancer a déjà livré des fichiers sur ce contrat
        $filesCheck = $pdo->prepare('
            SELECT COUNT(*) FROM contract_files WHERE contract_id = ? AND uploaded_by = ?
        ');
        $filesCheck->execute([$contractId, $contract['freelancer_id']]);
        $hasDelivery = (int)$filesCheck->fetchColumn() > 0;

        if ($hasDelivery) {
            // Livraison détectée -> PAS d'annulation ni de remboursement automatique.
            // On passe en litige pour vérification humaine avant toute décision.
            $pdo->prepare('
                UPDATE contracts
                SET status = "disputed", cancel_requested_by = ?, cancel_reason = ?
                WHERE id = ?
            ')->execute([$user['id'], $reason ?: 'Annulation demandée après livraison de fichiers par le freelancer.', $contractId]);

            sendNotification($contract['freelancer_id'], 'cancel_review', "Demande d'annulation en vérification",
                'Le client a demandé l\'annulation du contrat "' . $contract['project_title'] . '" alors qu\'une livraison a été détectée. Notre équipe va vérifier avant toute décision.',
                '/upc_freelance/app/contracts/details.php?id=' . $contractId);

            flash('warning', "Des fichiers ont déjà été livrés sur ce contrat. Votre demande d'annulation est transmise à notre équipe pour vérification — aucun remboursement automatique ne sera effectué avant leur décision.");
        } else {
            // Aucune livraison détectée -> annulation immédiate, comme avant
            $pdo->prepare('
                UPDATE contracts
                SET status = "cancelled", cancel_requested_by = ?, cancel_reason = ?
                WHERE id = ?
            ')->execute([$user['id'], $reason ?: null, $contractId]);

            $pdo->prepare('UPDATE projects SET status = "open" WHERE id = ?')->execute([$contract['project_id']]);
            $pdo->prepare('UPDATE wallets SET balance = balance + ?, locked = locked - ? WHERE user_id = ?')
                ->execute([$contract['amount'], $contract['amount'], $contract['client_id']]);
            recordTransaction($contract['client_id'], 'unlock', $contract['amount'], $contractId,
                'Montant libéré suite à annulation contrat #' . $contractId);
            sendNotification($contract['freelancer_id'], 'contract_cancelled', 'Contrat annulé',
                'Le contrat "' . $contract['project_title'] . '" a été annulé par le client.');
            flash('info', 'Contrat annulé. Le montant a été remis dans votre wallet.');
        }
    }

    // ─── Signaler un problème (plainte, sans bloquer le contrat) ──
    if ($action === 'report' && in_array($contract['status'], ['active', 'disputed'], true)) {
        if (!empty($reason)) {
            $pdo->prepare('
                INSERT INTO contract_reports (contract_id, reporter_id, reason)
                VALUES (?, ?, ?)
            ')->execute([$contractId, $user['id'], $reason]);
            flash('success', 'Votre signalement a été transmis à notre équipe. Le contrat continue normalement en attendant.');
        } else {
            flash('error', 'Merci de décrire le problème rencontré.');
        }
    }

    redirect('../../app/contracts/details.php?id=' . $contractId);
}

$pageTitle = 'Gérer le contrat — UPC Freelance';
$appLayout = true;
require_once '../../includes/header.php';
?>
<br>
<div class="mb-8">
    <a href="/upc_freelance/app/contracts/details.php?id=<?= $contractId ?>" class="inline-flex items-center gap-1 text-sm text-secondary hover:underline mb-3">
        <span class="material-symbols-outlined text-base">arrow_back</span> Retour au contrat
    </a>
    <h1 class="text-2xl font-bold text-primary">Gérer le contrat</h1>
    <p class="text-on-surface-variant text-sm mt-1"><?= h($contract['project_title']) ?></p>
</div>

<?php renderFlash(); ?>

<div class="max-w-xl space-y-4">

    <?php if ($contract['status'] === 'active'): ?>

    <!-- Ouvrir un litige -->
    <div class="bg-white rounded-2xl border border-amber-200 p-6 custom-shadow-low">
        <div class="flex items-start gap-3 mb-4">
            <span class="material-symbols-outlined text-amber-500 text-2xl">warning</span>
            <div>
                <h3 class="font-semibold text-primary">Ouvrir un litige</h3>
                <p class="text-sm text-on-surface-variant mt-1">
                    Si vous rencontrez un problème avec votre partenaire, vous pouvez ouvrir un litige. Notre équipe examinera la situation et prendra une décision. Le chat passera en lecture seule.
                </p>
            </div>
        </div>
        <form method="POST" class="space-y-3">
            <?= csrfField() ?>
            <input type="hidden" name="action" value="dispute"/>
            <div>
                <label class="block text-sm font-medium text-primary mb-1.5">Raison du litige <span class="text-red-500">*</span></label>
                <textarea name="reason" rows="3" required
                          placeholder="Décrivez le problème rencontré..."
                          class="w-full px-4 py-3 rounded-xl border border-outline-variant focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none text-sm resize-y"></textarea>
            </div>
            <button type="submit"
                    class="w-full bg-amber-500 text-white font-button text-button py-3 rounded-xl hover:bg-amber-600 transition-colors active:scale-95"
                    onclick="return confirm('Confirmer l\'ouverture du litige ?')">
                <span class="material-symbols-outlined align-middle mr-1">flag</span>
                Ouvrir un litige
            </button>
        </form>
    </div>

    <!-- Signaler un problème (sans bloquer le contrat) -->
    <div class="bg-white rounded-2xl border border-slate-200 p-6 custom-shadow-low">
        <div class="flex items-start gap-3 mb-4">
            <span class="material-symbols-outlined text-secondary text-2xl">report</span>
            <div>
                <h3 class="font-semibold text-primary">Signaler un comportement</h3>
                <p class="text-sm text-on-surface-variant mt-1">
                    Vous voulez alerter notre équipe sans bloquer le contrat ? Faites un signalement — le contrat continue normalement, mais notre équipe est informée.
                </p>
            </div>
        </div>
        <form method="POST" class="space-y-3">
            <?= csrfField() ?>
            <input type="hidden" name="action" value="report"/>
            <div>
                <label class="block text-sm font-medium text-primary mb-1.5">Description <span class="text-red-500">*</span></label>
                <textarea name="reason" rows="3" required
                          placeholder="Décrivez ce que vous souhaitez signaler..."
                          class="w-full px-4 py-3 rounded-xl border border-outline-variant focus:border-secondary focus:ring-2 focus:ring-secondary/20 outline-none text-sm resize-y"></textarea>
            </div>
            <button type="submit"
                    class="w-full bg-white text-secondary border-2 border-secondary/30 font-button text-button py-3 rounded-xl hover:bg-secondary/5 transition-colors active:scale-95">
                <span class="material-symbols-outlined align-middle mr-1">send</span>
                Envoyer le signalement
            </button>
        </form>
    </div>

    <!-- Annuler (client seulement) -->
    <?php if ($isClient): ?>
    <div class="bg-white rounded-2xl border border-red-200 p-6 custom-shadow-low">
        <div class="flex items-start gap-3 mb-4">
            <span class="material-symbols-outlined text-red-500 text-2xl">cancel</span>
            <div>
                <h3 class="font-semibold text-primary">Annuler le contrat</h3>
                <p class="text-sm text-on-surface-variant mt-1">
                    Si aucun fichier n'a encore été livré, le montant bloqué (<?= money((float)$contract['amount']) ?>)
                    sera remis dans votre wallet immédiatement. <strong>Si une livraison a déjà eu lieu, votre demande sera
                    vérifiée par notre équipe avant tout remboursement</strong>, afin d'éviter les annulations abusives après réception du travail.
                </p>
            </div>
        </div>
        <form method="POST" class="space-y-3">
            <?= csrfField() ?>
            <input type="hidden" name="action" value="cancel"/>
            <div>
                <label class="block text-sm font-medium text-primary mb-1.5">Raison de l'annulation <span class="text-red-500">*</span></label>
                <textarea name="reason" rows="3" required
                          placeholder="Pourquoi souhaitez-vous annuler ce contrat ?"
                          class="w-full px-4 py-3 rounded-xl border border-outline-variant focus:border-red-400 focus:ring-2 focus:ring-red-400/20 outline-none text-sm resize-y"></textarea>
            </div>
            <button type="submit"
                    class="w-full bg-red-50 text-red-600 border border-red-200 font-button text-button py-3 rounded-xl hover:bg-red-100 transition-colors active:scale-95"
                    onclick="return confirm('Êtes-vous sûr de vouloir annuler ce contrat ?')">
                <span class="material-symbols-outlined align-middle mr-1">close</span>
                Annuler le contrat
            </button>
        </form>
    </div>
    <?php endif; ?>

    <?php else: ?>
    <div class="bg-white rounded-2xl border border-slate-100 p-12 text-center custom-shadow-low">
        <span class="material-symbols-outlined text-4xl text-slate-300 block mb-3">lock</span>
        <p class="text-on-surface-variant">
            Ce contrat est <strong><?= $contract['status'] ?></strong> — aucune action disponible.
        </p>
        <?php if ($contract['status'] === 'disputed'): ?>
        <p class="text-xs text-slate-400 mt-2">
            <?= $contract['cancel_requested_by'] ? "Une demande d'annulation est en cours de vérification par notre équipe." : 'Un litige est en cours d\'examen par notre équipe.' ?>
        </p>
        <?php endif; ?>
        <a href="/upc_freelance/app/contracts/details.php?id=<?= $contractId ?>"
           class="mt-4 inline-block text-sm text-secondary hover:underline">
            Retour au contrat
        </a>
    </div>
    <?php endif; ?>
</div>

<?php $appLayout = true; require_once '../../includes/footer.php'; ?>